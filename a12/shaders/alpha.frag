...
// Task 1.6: alpha-masked foliage. Same PBR model as default.frag, but the base
// color texture's alpha channel encodes an alpha mask: fragments whose alpha is
// below the threshold are discarded. Foliage is two-sided, so the normal is
// flipped on back faces for plausible lighting.
...
void main()
{
	// Alpha masking: the alpha channel of the base color texture is the mask
	// (a12-bake: alphaMaskTexturePath == baseColorTexturePath). Sample once and
	// discard before doing any shading work.
	const vec4 base = texture( baseColor, vUV );
	if( base.a < 0.5 )
		discard;
	const vec3 cmat = base.rgb;
	...
	// Two-sided: flip the normal on back faces so the far side of a leaf is
	// lit as well.
	const vec3 n = normalize( vNormal ) * ( gl_FrontFacing ? 1.0 : -1.0 );
